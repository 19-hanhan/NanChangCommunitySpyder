...
def GetDataByHtml(html):
    ...
    for item in soup.find_all("div", class_="list_mod2"):
        ...
        # 创建正则表达式对象用于获取想要的信息
        findTitle = re.compile(r'<dt>.*<a.*>(.*?)</a>.*</dt>', re.S)  # 景点名称
        findLink = re.compile(r'<a class="score" href="(.*?)">')  # 景点详情链接
        findScore = re.compile(r'<a.*class="score".*><strong>(.*?)</strong>.*</a>') # 评分
        findCommentNum = re.compile(r'<a.*class="recomment".*>[ \f\r\t\n]*\((.*?)条点评\)</a>') # 评论数量
        findAddress = re.compile(r'<dd class="ellipsis">[ \f\r\t\n]*(.*?)[ \f\r\t\n]*</dd>')
